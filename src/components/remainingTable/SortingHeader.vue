<template>
  <th class="user-select-none" @click="emitSorting()">
    <span class="me-1">
      {{ title }}
    </span>

    <template v-if="currentSorting.column === column">
      <template v-if="type === 'NUMERIC'">
        <svg v-if="currentSorting.direction === 1" xmlns="http://www.w3.org/2000/svg" width="16"
             height="16" fill="currentColor" class="bi bi-sort-numeric-down" viewBox="0 0 16 16">
      ...
      <template v-else-if="type === 'LEXICOGRAPHIC'">
        <svg v-if="currentSorting.direction === 1" xmlns="http://www.w3.org/2000/svg" width="16"
          ...
          <path
              d="M12.96 14H9.028v-.691l2.579-3.72v-.054H9.098v-.867h3.785v.691l-2.567 3.72v.054h2.645V14zM4.5 2.5a.5.5 0 0 0-1 0v9.793l-1.146-1.147a.5.5 0 0 0-.708.708l2 1.999.007.007a.497.497 0 0 0 .7-.006l2-2a.5.5 0 0 0-.707-.708L4.5 12.293V2.5z"></path>
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
export default defineComponent({
  ...
  props: {
    ...
    column: {type: String as PropType<SortingColumn>, required: true},
    type: {type: String as PropType<"NUMERIC" | "LEXICOGRAPHIC">, required: true},
    currentSorting: {type: Object as PropType<Sorting>, required: true}
  },
  methods: {
    emitSorting(): void {
      const sorting: Sorting = {
        column: this.column,
        direction: this.currentSorting.direction === -1 ? 1 : -1
      }
      this.$emit('sortingUpdated', sorting);
    }
  },
});
</script>
